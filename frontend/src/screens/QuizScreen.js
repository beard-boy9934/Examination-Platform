...
const QuizScreen = ({ history, match }) => {
  ...
  const handleKeyDown = (event) => {
    ...
    if (event.keyCode === 17) {
      ...
      console.log(attempts)
      alert(
        'You tried to Copy a content. It is Forbidenfor this Test. If you attempt again, your test would be ended.'
      )
    }
  }
  ...
  const handleKeyUp = (event) => {
    ...
    if (event.keyCode === 44) {
      ...
      console.log(attempts)
      alert(
        'You tried to Take ScreenShot. It is Forbiden for this Test. If you attempt again, your test would be ended.'
      )
    }
    if (event.keyCode === 91) {
      count += 1
  ...
  // lower = 725,upper = 150
  const handleOver = (e) => {
    if (e.screenY < 150 || e.screenY > 725) {
      count += 1
      setAttempts(count)
      console.log(attempts)
